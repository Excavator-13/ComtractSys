        ...
        <tbody>
          <tr v-for="c in contracts" :key="c.id">
            ...
            <td class="row-actions">
              ...
              <button v-if="hasPermission('contract:update') && Number(c.drafterId) === Number(auth.user?.id) && (c.status === 'DRAFT' || c.status === 'COUNTERSIGNED' || c.status === 'RETURNED')" @click="editContract(c)"><Pencil :size="14" /> 编辑</button>
