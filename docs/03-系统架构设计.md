...
后端使用 `@RequirePermission` 标注受保护接口，由统一拦截器读取当前登录用户的角色和权限点进行校验。一个接口可以声明多个权限码，用户拥有其中任意一个即可访问。
...
1. 密码使用 BCrypt 加密存储。
2. 当前开发阶段登录成功返回服务端内存 Token，前端通过 `Authorization: Bearer <token>` 调用接口；后续可平滑替换为 JWT。
3. 后端接口使用角色和权限点控制，权限拦截在 Controller 前统一执行。
